...
//public static class RelationalDependencyChecker
//{
//    public static async Task<bool> HasRelationalDependenciesAsync<TEntity>(
//        TEntity entity
//    ) where TEntity : class
//    {
...
//        // 1. Discover table name
//        string tableName = await GetTableNameAsync(connectionString, entityType.Name);
//        if (string.IsNullOrWhiteSpace(tableName))
//            throw new Exception($"Table for entity {entityType.Name} not found.");
...
//        // 3. Extract primary key values from entity
//        var keyValues = new Dictionary<string, object>();
//        foreach (var col in pkColumns)
//        {
//            var prop = entityType.GetProperty(col, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
//            if (prop == null)
//                throw new Exception($"Property '{col}' not found on entity '{entityType.Name}'.");

//            var val = prop.GetValue(entity);
//            if (val == null)
//                throw new Exception($"Primary key property '{col}' is null.");

//            keyValues[col] = val;
//        }

//        // 4. Discover foreign key relationships
//        var relatedTables = await GetForeignKeyDependentsAsync(conn, tableName);

//        // 5. Check each related table
//        foreach (var rel in relatedTables)
//        {
//            string whereClause = string.Join(" AND ", rel.ForeignKeyColumns.Select(col => $"[{col}] = @{col}"));
//            string query = $@"
//                SELECT COUNT(1)
//                FROM [{rel.TableName}]
//                WHERE {whereClause}
...
//            foreach (var col in rel.ForeignKeyColumns)
//                cmd.Parameters.AddWithValue("@" + col, keyValues[col]);
...
//    private static async Task<string> GetTableNameAsync(string connStr, string entityName)
//    {
...
//            WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME LIKE @Name", conn);
...
//    private static async Task<List<(string TableName, List<string> ForeignKeyColumns)>> GetForeignKeyDependentsAsync(SqlConnection conn, string principalTable)
//    {
...
//            ORDER BY DependentTable, FKColumn", conn);

//        cmd.Parameters.AddWithValue("@TableName", principalTable);
...
static class RelationalDependencyChecker
{
    ...
    public static async Task CreateOpenContextAsync()
    {
        ...
        //string con = Con+ ";Password=123456";
        //var options = new DbContextOptionsBuilder<DBContext>()
        //    .UseSqlServer(con, x => x.UseNetTopologySuite())
        //    .Options;
